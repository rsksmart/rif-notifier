...
public interface TopicRepository extends JpaRepository<Topic, String> {
    ...
    @Query(value = "SELECT distinct * FROM topic A JOIN user_topic B ON A.id=B.id_topic JOIN subscription C ON c.id=B.id_subscription WHERE c.active=1 and c.notification_balance > 0", nativeQuery = true)
